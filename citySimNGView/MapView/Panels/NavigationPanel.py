...
class NavigationPanel(Panel):
    ...
    def __init__(self, pos_x, pos_y, width, height, blit_surface, action):
        """ Constructor.
        ...
        """
        Panel.__init__(self, pos_x, pos_y, width, height, NAV_PANEL_TEX, blit_surface, 'Navigation Panel')
        self.action = action
        ...
        self.left_arrow = Button(self.pos_x, middle_y, self.width * NAV_ARROW_WIDTH, self.height * NAV_ARROW_HEIGHT,
                                 NAV_ARROW_TEXTURE, self.action, self, "Go left", texture_rotation=0)
        self.up_arrow = Button(middle_x, self.pos_y, self.width * NAV_ARROW_HEIGHT, self.height * NAV_ARROW_WIDTH,
                               NAV_ARROW_TEXTURE, self.action, self, "Go up", texture_rotation=270)
        self.right_arrow = Button(max_x, middle_y, self.width * NAV_ARROW_WIDTH, self.height * NAV_ARROW_HEIGHT,
                                  NAV_ARROW_TEXTURE, self.action, self, "Go right", texture_rotation=180)
        self.down_arrow = Button(middle_x, max_y, self.width * NAV_ARROW_HEIGHT, self.height * NAV_ARROW_WIDTH,
                                 NAV_ARROW_TEXTURE, self.action, self, "Go down", texture_rotation=90)
    ...
    def draw(self):
        ...
        being cleaned. """
        self.clean()
        draw_text(0, 0, "{}:{}".format(self.map_position_x, self.map_position_y), PURPLE, self.surface)
        for arrow in self.navigation_arrows_sprites:
            self.surface.blit(arrow.image, (arrow.rect[0] - self.pos_x, arrow.rect[1] - self.pos_y))
