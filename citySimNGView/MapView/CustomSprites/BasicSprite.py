...
class BasicSprite(pygame.sprite.DirtySprite):
    ...
    def __init__(self, pos_x, pos_y, width, height, texture_path, popup_text, texture_rotation=0, rect_point='topleft'):
        ...
        self.load_texture(texture_rotation, rect_point)

    def load_texture(self, texture_rotation=0, rect_point='topleft'):
        """ Load texture from file, scale it and get it's rect. """
        try:
            self.image = pygame.image.load(self.texture_path)
        except Exception:
            print 'Could not find texture: {}. Loading default texture.'.format(self.texture_path)
